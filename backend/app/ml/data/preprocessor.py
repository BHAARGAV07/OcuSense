...
class OcuSensePreprocessor:
    ...
    def canonical_to_features_df(self, canonical: PredictionFeatures) -> pd.DataFrame:
        ...
        row_dict = {
            ...
            "Redness_Score": optional_float(ocular.redness_score),
            "Inflammation_Score": optional_float(ocular.inflammation_score),
            "Swelling_Score": optional_float(ocular.swelling_score),
            "Watering_Score": optional_float(ocular.tear_feature),
            "Image_Quality_Score": optional_float(ocular.image_quality),
            "PM2.5": optional_float(env.pm25),
            "PM10": optional_float(env.pm10),
            "NO2": optional_float(env.no2),
            "O3": optional_float(env.o3),
            "AQI": optional_float(env.aqi),
            "Temperature": optional_float(env.temperature),
            "Humidity": optional_float(env.humidity),
            "Pollen_Index": optional_float(env.pollen_index),
